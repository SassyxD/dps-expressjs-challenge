...
router.post('/:tournamentId/games', (req: Request, res: Response) => {
	try {
		...
		if (!player1_id || !player2_id || player1_score === undefined || player2_score === undefined) {
			res.status(400).json({ error: 'All fields are required: player1_id, player2_id, player1_score, player2_score' });
			return;
		}

		if (player1_id === player2_id) {
			res.status(400).json({ error: 'A player cannot play against themselves' });
			return;
		}
		...
		const participant1 = db.query(
			...
			{ 1: tournamentId, 2: player1_id }
		);

		const participant2 = db.query(
			'SELECT * FROM tournament_participants WHERE tournament_id = ? AND player_id = ?',
			{ 1: tournamentId, 2: player2_id }
		);

		if (!participant1 || participant1.length === 0) {
			res.status(400).json({ error: 'Player 1 is not in this tournament' });
			return;
		}

		if (!participant2 || participant2.length === 0) {
			res.status(400).json({ error: 'Player 2 is not in this tournament' });
			return;
		}
		...
		const existingGame = db.query(
			...
			{ 1: tournamentId, 2: player1_id, 3: player2_id, 4: player2_id, 5: player1_id }
		);

		if (existingGame && existingGame.length > 0) {
			res.status(400).json({ error: 'Game between these players already recorded' });
			return;
		}
		...
		db.run(
			...
			{ 1: tournamentId, 2: player1_id, 3: player2_id, 4: player1_score, 5: player2_score }
		);
		...
		db.run(
			'UPDATE tournament_participants SET points = points + ? WHERE tournament_id = ? AND player_id = ?',
			{ 1: player2Points, 2: tournamentId, 3: player2_id }
		);
